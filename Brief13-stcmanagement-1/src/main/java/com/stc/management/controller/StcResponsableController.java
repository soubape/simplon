...
public class StcResponsableController {
	...
	@GetMapping 
	 ...
	 // build get employee by id REST API
	    ...
	    @PutMapping("{id}")
	    public ResponseEntity<Responsable> updateResponsable(@PathVariable long id,@RequestBody Responsable exerciceDetails) {
	    	Responsable responsable =  responsablerepository.findById(id)
	                .orElseThrow(() -> new ResourceNotFoundException("Responsable not exist with id: " + id));

	    	responsable.setDomaine(exerciceDetails.getDomaine());
	    	responsable.setEmail(exerciceDetails.getEmail());
	    	responsable.setEtat(exerciceDetails.isEtat());
	    	responsable.setName(exerciceDetails.getName());
	    	responsable.setTelephone(exerciceDetails.getTelephone());
	    	responsable.setTyperesponsable(exerciceDetails.getTyperesponsable());
